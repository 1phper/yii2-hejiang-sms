...
class AlidayuSender extends BaseSender implements SenderInterface
{
    ...
    {
        $client = new Client(new App(['app_key' => $this->accessKey, 'app_secret' => $this->secretKey]));
        $request = new AlibabaAliqinFcSmsNumSend();
        $request->setRecNum($phoneNumber)->setSmsTemplateCode($tplId)->setSmsParam($tplParams)->setSmsFreeSignName($sign);
